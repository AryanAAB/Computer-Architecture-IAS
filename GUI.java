...
public class IMT2023513_GUI extends JFrame
{
    ...
    {
        IMT2023513_GUI gui = new IMT2023513_GUI();
...
{
    private IMT2023513_OutputReader read;                       //Stores a reader object which reads the output file.
    ...
    {
        this.setBackground(BACKGROUND_COLOR);
        this.setFont(IMT2023513_GUI.DEFAULT_FONT);
        this.setText("NEXT");

        read = new IMT2023513_OutputReader("Output.txt");
    ...
    public void setStartingValues()
    {
        ...
        IMT2023513_GUI.getManager().updateStage(list.remove(0));
        IMT2023513_GUI.getManager().updateRegisters(list);
    }
    ...
    public void actionPerformed(ActionEvent e) 
    {
        ...
        IMT2023513_GUI.getManager().reset();

        //List is null impliex exit.
        if(list == null)
        {
            IMT2023513_GUI.getManager().popUp(read.readEnding());
        }
        //Size is one implies that there is a state change.
        else if(list.size() == 1)
        {
            IMT2023513_GUI.getManager().updateStage(list.remove(0));
            IMT2023513_GUI.getManager().updateOperation("Changing stage");
        }
        else
        {
            IMT2023513_GUI.getManager().updateOperation(list.remove(0));

            //Size is 1 after removal implies that this is a read instruction.
            if(list.size() == 1)
            {
                IMT2023513_GUI.getManager().read(list.remove(0));
            }
            //Size is 2 after removal implies that this is a write instruction. 
            else if(list.size() == 2)
            {
                IMT2023513_GUI.getManager().write(list.remove(0));
            }
...
class Stage extends JPanel
{
    ...
    public Stage()
    {
        ...
        this.label.setFont(IMT2023513_GUI.DEFAULT_FONT);
        this.add(label);
    }
...
class EAST extends JPanel
{
    ...
    public EAST()
    {
        ...
        label.setFont(IMT2023513_GUI.DEFAULT_FONT);
...
class EASTCenter extends JPanel
{
    ...
    protected void paintComponent(Graphics g)
    {
        ...
        g.setFont(IMT2023513_GUI.DEFAULT_FONT);
        ...
        for(int i = 0; i < 7; i++)
        {
            ...
            TextLayout layout = new TextLayout(labels[i], IMT2023513_GUI.DEFAULT_FONT, frc);   //used for drawing the object.
            layout.draw(g2d, x, y);
            y += 20;
        }
    }
